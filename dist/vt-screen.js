    ...
    write(data) {
        ...
        while (i < data.length) {
            ...
            if (ch === "\x1b" && i + 1 < data.length) {
                ...
                i += 2;
                if (i < data.length && /[()#]/.test(data[i - 1]))
                    i++;
                continue;
            }
            ...
            if (ch === "\t") {
                this.curCol = Math.min(this.curCol + (8 - (this.curCol % 8)), this.cols - 1);
                i++;
                continue;
            }
            if (ch === "\x08") {
            ...
            case "J": // ED: Erase in Display
                if (args[0] === 2 || args[0] === 3) {
                    ...
                    for (let r = 0; r < this.curRow; r++)
                        this.grid[r].fill(" ");
                    for (let c = 0; c <= this.curCol && c < this.cols; c++)
                        this.grid[this.curRow][c] = " ";
                }
                else {
 ...
 */
export function replayToScreen(hexDataChunks, rows = 24, cols = 80) {
...
//# sourceMappingURL=vt-screen.js.map