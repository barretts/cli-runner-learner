...
export function initState(manifest, policy) {
    const tasks = {};
 ...
 */
export async function loadOrInitState(statePath, manifest, policy) {
    let existing = null;
    try {
    ...
    if (!existing)
        return initState(manifest, policy);
...
export async function checkpoint(statePath, state) {
