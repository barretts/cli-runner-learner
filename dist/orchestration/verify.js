...
export async function runVerification(driveResult, task, profile) {
    const results = [];
...
async function runStep(step, driveResult, task) {
    ...
    switch (step.check) {
        ...
        case "command": {
            ...
            try {
                const result = await runCommand(step.command, task.work_dir, (step.timeout_sec ?? 30) * 1000);
                return {
...
function runCommand(command, cwd, timeout_ms = 30_000) {
    return new Promise((resolve, reject) => {
        const proc = execFile("sh", ["-c", command], { cwd: cwd ?? process.cwd(), timeout: timeout_ms, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
