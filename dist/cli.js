...
import { parseTranscript, segmentByGaps, segmentByFrames, computeTimingProfile } from "./engine/transcript.js";
import { stripTermEscapes, extractDiagnosticLines } from "./term-utils.js";
import { classifySegments } from "./engine/classifier.js";
...
import { planNextProbe } from "./engine/probe-planner.js";
import { diagnoseLearnFailures, heal, applyHealPatches } from "./engine/healer.js";
import { initLearnState, checkpointLearnState, loadLearnState } from "./engine/learn-state.js";
...
program
    ...
    if (llmClient)
        console.log("[discover] LLM enabled for structured parsing");
...
program
    ...
    const interactionMode = opts.mode;
    const healMode = opts.heal;
    ...
    if (llmClient)
        console.log(`[learn] LLM enabled (budget: ${llmClient.getUsage().calls} / call limit)`);
    ...
    const confidenceHistory = [...learnState.confidence_history];
    let healerSuggestion;
    ...
    for (let round = startRound; round < maxRounds; round++) {
        ...
        if (planned.expected_outcome)
            console.log(`[learn] Expected: ${planned.expected_outcome}`);
        const config = createSessionConfig({
            command,
            args,
        ...
        if (healMode !== "off" &&
            ...
            for (const d of diagnosis)
                console.log(`  [${d.failure_class}] ${d.detail}`);
            ...
            const isNonConvergent = prev2 !== null &&
                signatures.length > 0 &&
                prev2.every((hr) => hr.failure_signatures.length === signatures.length &&
    ...
    if (healRoundsUsed > 0) {
        ...
        if (learnState.abort_reason)
            console.log(`[learn] Abort reason: ${learnState.abort_reason}`);
    }
    if (llmClient) {
        const usage = llmClient.getUsage();
...
program
    ...
    if (workDir)
        console.log(`[run] Tracking side effects: ${workDir}`);
    ...
    if (result.state_diff) {
        ...
        if (diff.new_files.length > 0)
            console.log(`[run]   New: ${diff.new_files.join(", ")}`);
        if (diff.modified_files.length > 0)
            console.log(`[run]   Modified: ${diff.modified_files.join(", ")}`);
        if (diff.deleted_files.length > 0)
            console.log(`[run]   Deleted: ${diff.deleted_files.join(", ")}`);
    }
    console.log(`[run] Output:\n`);
    console.log(result.output);
});
// ---- Shared helpers ----
async function executeProbeStrategy(session, strategy, inputText, maxMs) {
    switch (strategy) {
        case "observe":
...
async function waitForSettled(session, maxMs) {
...
async function waitForSettledAndExit(session, maxMs) {
    ...
    if (!session.done) {
        ...
        if (!session.done)
            console.log(`[wait] Exit wait timed out after ${exitWaitEvents} events`);
    }
    else {
...
program
    ...
    if (llmClient)
        console.log(`[orch] Healer LLM: budget=${opts.llmBudget}`);
