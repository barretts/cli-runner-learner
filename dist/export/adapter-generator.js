 ...
 */
export function loadAdapterOverrides() {
    if (_overridesCache)
 ...
 */
export function profileToAdapterPreset(profile, useOverrides = true) {
    const discovery = profile.discovery;
    // Map interaction mode to prompt delivery
    let promptDelivery = "stdin";
    ...
    transientPatterns.push("stream error", "ECONNREFUSED", "ECONNRESET", "ETIMEDOUT", "429", "rate limit");
    // Session management detection
    let sessionShowCommand;
    let sessionIdPattern;
    let sessionContinueFlag;
    ...
    if (ov.promptDelivery)
        base.promptDelivery = ov.promptDelivery;
    if (ov.promptFlag)
        base.promptFlag = ov.promptFlag;
    if (ov.stdinIgnore !== undefined)
        base.stdinIgnore = ov.stdinIgnore;
    if (ov.toolCallsHiddenInStdout !== undefined)
        base.toolCallsHiddenInStdout = ov.toolCallsHiddenInStdout;
    if (ov.needsLineBuffering !== undefined)
        base.needsLineBuffering = ov.needsLineBuffering;
    if (ov.maxTurns !== undefined)
        base.maxTurns = ov.maxTurns;
    // Array overrides (replace entirely when present)
    if (ov.defaultArgs)
        base.defaultArgs = ov.defaultArgs;
    if (ov.noisePatterns)
        base.noisePatterns = ov.noisePatterns;
    if (ov.notes)
        base.notes = ov.notes;
 ...
 */
export function generateAdapterTypeScript(profile, useOverrides = true) {
 ...
 */
export function generateAdapterJSON(profile, useOverrides = true) {
    const preset = profileToAdapterPreset(profile, useOverrides);
    return JSON.stringify(preset, null, 2);
}
//# sourceMappingURL=adapter-generator.js.map