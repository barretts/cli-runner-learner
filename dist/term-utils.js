 ...
 */
const TERM_ESCAPE_RE = /\x1b(?:\[[0-9;?]*[A-Za-z~]|\][^\x07]*\x07|\][^\x1b]*\x1b\\|[^\[\]].?)|\x9b[0-9;?]*[A-Za-z~]|[\x00-\x08\x0b-\x1f\x7f]/g;
export function stripTermEscapes(s) {
    return s.replace(TERM_ESCAPE_RE, "");
}
export function hasVisibleContent(line) {
 ...
 */
const TUI_ARTIFACT_RE = /\[[?>=]*\d*[;?\d]*(?:\$[a-zA-Z]|[a-zA-Z])|\[\d+ [a-zA-Z]|_G[^\\]*\\?|i=\d+[,;][^\s]*|AAAA|\x00+|[\x80-\x9f]|[\u2500-\u257f]|[\u2580-\u259f]|[\u2800-\u28ff]/g;
export function deepStripTuiArtifacts(text) {
 ...
 */
export function extractDiagnosticLines(raw) {
 ...
 */
export function globMatch(pattern, text, caseInsensitive = false) {
    let p = pattern;
    let t = text;
    if (caseInsensitive) {
        p = p.toLowerCase();
