...
export class Session {
    ...
    nextEvent(timeout_ms) {
        ...
        return new Promise((resolve) => {
            const timer = setTimeout(() => {
                ...
                if (idx !== -1)
                    this.eventResolvers.splice(idx, 1);
    ...
    resetSettleTimer() {
        ...
        this.settleTimer = setTimeout(() => {
            if (this._done || this.settledEmitted)
                return;
    ...
    async writeTranscript(type, fields) {
        if (!this.transcriptFd)
            return;
        const ts = Date.now();
        const obj = { ts, type, ...fields };
        try {
