 ...
 */
export function extractPatterns(classifiedRuns) {
    ...
    for (const run of classifiedRuns) {
        ...
        for (const seg of run.segments) {
            if (seg.state === "unknown" || seg.state === "completed") {
                skippedUnknown++;
                continue;
            }
            ...
            for (const frag of fragments) {
                ...
                for (const [otherKey, otherCand] of candidates) {
                    if (otherKey !== key && otherCand.pattern === frag) {
                        ...
                        if (c)
                            c.all_states.add(otherCand.state);
                    }
                }
            }
        }
...
function looksRandom(text) {
    ...
    if (text.length <= 20 && uniqueRatio > 0.85)
        return true;
    return false;
}
/**
...
function toGlobPattern(text) {
    ...
    if (pattern === "*")
        return text.substring(0, MAX_PATTERN_LENGTH);
    return pattern;
}
function limitPerState(patterns, max) {
...
function deduplicatePatterns(patterns) {
    ...
    for (const pat of patterns) {
        ...
        for (const existing of keep) {
            if (existing.classified_as !== pat.classified_as)
                continue;
        ...
        if (!dominated) {
            for (let i = keep.length - 1; i >= 0; i--) {
                if (keep[i].classified_as === pat.classified_as &&
