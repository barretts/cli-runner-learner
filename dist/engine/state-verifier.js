...
async function isGitRepo(workDir) {
    try {
 ...
 */
export async function captureState(workDir) {
 ...
 */
export async function compareStates(workDir, before, after) {
    let rawDiff = "";
    const newFiles = [];
    const modifiedFiles = [];
    const deletedFiles = [];
    ...
    if (modifiedFiles.length > 0)
        summaryParts.push(`${modifiedFiles.length} modified`);
    if (deletedFiles.length > 0)
        summaryParts.push(`${deletedFiles.length} deleted`);
 ...
 */
export async function initStateTracking(workDir) {
    if (await isGitRepo(workDir))
        return;
 ...
 */
export async function checkpointState(workDir, label) {
    await git(workDir, ["add", "-A"]);
    try {
 ...
 */
export async function rollbackToCheckpoint(workDir, commitHash) {
