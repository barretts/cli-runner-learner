 ...
 */
export async function classifySegments(segments, profile, llmClient) {
    ...
    const results = [];
    const hasExit = segments.some((s) => s.events.some((e) => e.type === "meta" && e.event === "exit"));
...
function classifyOne(seg, ctx, profile) {
    ...
    const activeStates = ["startup", "working", "thinking", "prompting"];
 ...
 */
function matchProfileIndicators(text, profile) {
    let checkedCount = 0;
    const matches = [];
 ...
 */
export function extractTextForState(classified, state) {
    const lines = new Set();
