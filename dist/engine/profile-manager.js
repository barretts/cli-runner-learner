 ...
 */
export async function loadProfile(toolId) {
...
export function bootstrapProfile(toolId, command, interactionMode = "interactive", discovery) {
 ...
 */
export function mergeLearnedPatterns(profile, newPatterns, metadata) {
    ...
    for (const pattern of newPatterns) {
        ...
        const existing = updated.learned_patterns.find((p) => p.pattern === pattern.pattern && p.classified_as === pattern.classified_as);
        if (existing) {
        ...
        if (effectiveConfidence >= 0.5) {
            ...
            if (stateName in updated.states) {
                ...
                const alreadyExists = stateDef.indicators.some((ind) => ind.type === "output_glob" && ind.pattern === pattern.pattern);
                if (!alreadyExists) {
 ...
 */
export function registerSubPrompt(profile, detected) {
    const updated = structuredClone(profile);
    ...
    const existing = updated.states.prompting.sub_prompts.find((sp) => sp.indicators.some((ind) => ind.pattern === globPattern));
    if (existing)
        return updated;
 ...
 */
export function registerStructuralIndicators(profile, classifiedRuns) {
    const updated = structuredClone(profile);
    for (const run of classifiedRuns) {
        for (const seg of run.segments) {
            if (seg.state === "ready" && seg.confidence >= 0.5) {
                ...
                const hasStructural = readyState.indicators.some((ind) => ind.type === "silence_after_output_ms");
                if (!hasStructural) {
                    const silenceMs = seg.events.find((e) => e.type === "meta" && e.event === "settled")?.value;
...
function computeProfileConfidence(profile) {
    ...
    for (const stateName of patternStates) {
        ...
        const statePatterns = profile.learned_patterns.filter((p) => p.classified_as === stateName);
        if (statePatterns.length > 0) {
            const avgConf = statePatterns.reduce((sum, p) => sum + p.confidence, 0) / statePatterns.length;
            totalScore += avgConf;
