 ...
 */
export function stableLearnFailureSignature(failureClass, signal) {
...
// ---- Failure Diagnosis ----
const PATTERN_STATES = ["startup", "ready", "working", "thinking", "prompting"];
/**
 ...
 */
export function diagnoseLearnFailures(probes, profile) {
 ...
 */
export async function heal(ctx, llmClient) {
...
function deterministicHeal(ctx) {
    ...
    if (stateGaps.length > 0) {
        ...
        const suggestedProbes = [];
        for (const s of emptyStates) {
            suggestedProbes.push(probeForState(s, ctx.profile));
        }
        return {
            decision: "RETRY",
            failure_class: "state_gap",
    ...
    if (noOutput) {
        ...
        const suggestedProbes = subcommands.length > 0
            ? [{
                    strategy: "custom",
 ...
 */
function probeForState(state, profile) {
    switch (state) {
        case "ready":
            return {
                strategy: "enter",
 ...
 */
export function applyHealPatches(profile, decision, originalConfig) {
    ...
    for (const patch of decision.patches) {
        switch (patch.target) {
            case "classification_hint": {
                if (patternsAdded >= MAX_PATTERNS_PER_HEAL)
                    break;
                try {
                    const hint = JSON.parse(patch.content);
                    if (hint.state in updated.states &&
                        hint.pattern &&
                        hint.pattern.length <= MAX_PATTERN_LENGTH) {
                        const stateDef = updated.states[hint.state];
                        const alreadyExists = stateDef.indicators.some((ind) => ind.type === "output_glob" && ind.pattern === hint.pattern);
                        if (!alreadyExists) {
            ...
            case "timing_knob": {
                try {
                    const knobs = JSON.parse(patch.content);
            ...
            case "profile_state": {
                try {
                    const stateUpdate = JSON.parse(patch.content);
