...
function deterministicHeal(failed: FailedTaskSummary[]): OrchHealDecision {
  ...
  let dominantClass: OrchFailureClass = "prompt_gap";
  for (const f of failed) {
...
const VALID_FAILURE_CLASSES: Set<string> = new Set(ORCH_FAILURE_CLASSES);
const VALID_PATCH_TARGETS = new Set(["shared_context", "task_input", "timing"]);

function parseHealDecision(raw: string): OrchHealDecision | null {
  ...
  const failureClass = String(obj.failure_class ?? "") as OrchFailureClass;
  if (!VALID_FAILURE_CLASSES.has(failureClass)) return null;

  const patches: OrchHealPatch[] = [];
  if (Array.isArray(obj.patches)) {
    for (const p of obj.patches as Array<Record<string, unknown>>) {
      ...
      if (VALID_PATCH_TARGETS.has(target) && p.content) {
        patches.push({
          target: target as OrchHealPatch["target"],
          operation: (String(p.operation ?? "append") === "replace" ? "replace" : "append") as "append" | "replace",
  ...
  const escalations: Array<{ task_id: string; reason: string }> = [];
  if (Array.isArray(obj.escalations)) {
    for (const e of obj.escalations as Array<Record<string, unknown>>) {
      if (e.task_id) {
        escalations.push({
  ...
  return {
    decision: decision as OrchHealDecision["decision"],
    failure_class: failureClass as OrchFailureClass,
