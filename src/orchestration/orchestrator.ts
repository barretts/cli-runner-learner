...
import type { ToolProfile, DriveResult } from "../types.js";
...
import { selectAdapter, type OutputAdapter } from "./adapter.js";
...
import { diagnoseTaskFailure, healBatch } from "./healer.js";
...
export class Orchestrator {
      ...
      if (failedInBatch.length === 0) {
        ...
        if (this.totalHealRounds < this.policy.max_total_heal_rounds) {
          ...
          for (const t of failedInBatch) {
            const ts = this.state.tasks[t.id];
            if (ts && ts.status === "FAILED") ts.status = "ESCALATED";
          }
        }
      } else {
  ...
  private depsReady(task: TaskDef): boolean {
    for (const dep of task.depends_on) {
      const ts = this.state.tasks[dep];
      if (!ts || ts.status !== "DONE") return false;
    }
    return true;
  }

  private async executeTask(taskDef: TaskDef): Promise<void> {
    const taskState = this.state.tasks[taskDef.id];
    if (!taskState) return;

    if (taskState.status === "DONE" || taskState.status === "ESCALATED") return;
    ...
    if (taskDef.verify && this.manifest.verify_profiles?.[taskDef.verify]) {
      const verification = await runVerification(
        driveResult,
        taskDef,
        this.manifest.verify_profiles[taskDef.verify],
      );
  ...
  private async runHeal(failedTasks: TaskDef[]): Promise<boolean> {
    ...
    if (decision.decision === "RETRY") {
      ...
      for (const patch of decision.patches) {
        switch (patch.target) {
          case "shared_context":
            if (!this.state.shared_context_patches) this.state.shared_context_patches = [];
            this.state.shared_context_patches.push(patch.content);
            console.log(`[heal] Patched shared context (+${patch.content.length} chars)`);
            break;
          case "task_input":
            if (patch.task_id) {
              if (!this.state.prompt_patches) this.state.prompt_patches = {};
  ...
  private addSignature(ts: PerTaskState, sig: string): void {
    if (!ts.failure_signatures_seen) ts.failure_signatures_seen = [];
