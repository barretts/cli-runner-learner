import type { ToolProfile, StateDefinition, ClassifiedSegment, ProbeStrategy } from "../types.js";
...
export function buildClassifierPrompt(
  segmentText: string,
  profileStates: Record<string, StateDefinition>,
  context: { segmentIndex: number; totalSegments: number; prevState?: string },
): PromptPair {
...
export function buildToolDiscoveryPrompt(helpText: string): PromptPair {
  return {
...
export function buildProbeStrategyPrompt(
  profile: ToolProfile,
  probeHistory: ProbeHistoryEntry[],
): PromptPair {
  const stateConfidence: Record<string, string> = {};
  ...
  const historyText = probeHistory.length > 0
    ? probeHistory.map((h) =>
        `  Round ${h.round}: strategy="${h.strategy}"${h.input_text ? ` input="${h.input_text}"` : ""} -> states: [${h.states_observed.join(", ")}]`
      ).join("\n")
...
export function buildSubPromptAnalysisPrompt(outputText: string): PromptPair {
  return {
